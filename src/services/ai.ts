import type { Agent, Task, Client, AIProvider } from '../types';
...
[TASK_JSON]{"create_task": true, "title": "título da tarefa", "description": "descrição", "content_type": "post|carrossel|reels|stories|video", "channel": "instagram|facebook|tiktok|youtube|linkedin", "priority": "low|medium|high|urgent", "client_name": "nome do cliente se mencionado"}[/TASK_JSON]
...
export function extractTaskFromResponse(response: string, clients: Client[]): TaskExtractionResult {
  const taskMatch = response.match(/\[TASK_JSON\](.*?)\[\/TASK_JSON\]/s);
  if (!taskMatch) return { shouldCreateTask: false };

  try {
    const taskData = JSON.parse(taskMatch[1]);
    if (!taskData.create_task) return { shouldCreateTask: false };

    let clientId = clients[0]?.id || '';
    if (taskData.client_name) {
      const foundClient = clients.find(c => 
        c.name.toLowerCase().includes(taskData.client_name.toLowerCase()) ||
        c.company.toLowerCase().includes(taskData.client_name.toLowerCase())
      );
    ...
    return {
      ...
      task: {
        ...
        content_type: taskData.content_type || 'post',
        channel: taskData.channel || 'instagram',
...
export function cleanResponse(response: string): string {
  return response.replace(/\[TASK_JSON\].*?\[\/TASK_JSON\]/s, '').trim();
}
...
export async function trainAgentKnowledge(
  ...
  const trainedKnowledge = knowledgeItems
    .map(item => `=== ${item.name} ===\n${item.content}`)
    .join('\n\n---\n\n');
  return trainedKnowledge;
}
