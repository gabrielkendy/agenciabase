...
export const WorkflowPage = () => {
  const { 
    ...
    demandFilters, setDemandFilters, clearFilters 
  } = useStore();
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-3">
            <span className="text-3xl">📋</span> Workflow
          </h1>
          <p className="text-gray-400 text-sm mt-1">Gerencie demandas de conteúdo estilo mLabs</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex bg-gray-800 rounded-lg p-1">
            <button 
              onClick={() => setViewMode('kanban')} 
              className={clsx('px-3 py-1.5 rounded-md text-sm transition', viewMode === 'kanban' ? 'bg-orange-500 text-white' : 'text-gray-400 hover:text-white')}
            >
              📊 Kanban
            </button>
            <button 
              onClick={() => setViewMode('list')} 
              className={clsx('px-3 py-1.5 rounded-md text-sm transition', viewMode === 'list' ? 'bg-orange-500 text-white' : 'text-gray-400 hover:text-white')}
            >
              📋 Lista
            </button>
          </div>
          <button className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-gray-300 px-4 py-2 rounded-xl transition">
            <Icons.Filter size={18} /> Filtros
          </button>
          <button className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-gray-300 px-4 py-2 rounded-xl transition">
            <Icons.Settings size={18} /> Config
          </button>
          <button 
            onClick={openNewDemand} 
            className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-xl transition font-medium"
          >
            <Icons.Plus size={18} /> Adicionar demanda
          </button>
        </div>
      </div>

      {/* Filters Bar */}
      <div className="flex items-center gap-3 mb-6 flex-wrap bg-gray-800/50 rounded-xl p-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5">
            ...
            <span className="text-sm text-white">
              {new Date().toLocaleDateString('pt-BR')} - {new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString('pt-BR')}
            </span>
          </div>
        </div>
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input 
            ...
            className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-10 pr-4 py-1.5 text-sm text-white placeholder-gray-500 focus:border-orange-500 focus:outline-none" 
          />
        </div>
        
        {(demandFilters.clientId || demandFilters.status || demandFilters.search) && (
          <button onClick={clearFilters} className="text-sm text-orange-400 hover:text-orange-300 underline">
            Limpar filtros
          </button>
        )}
        ...
        <div className="flex gap-4 h-full min-w-max pb-4">
          {WORKFLOW_COLUMNS.map((column) => (
            <div 
              ...
              className={clsx(
                'w-72 flex-shrink-0 bg-gray-800/50 rounded-2xl flex flex-col', 
